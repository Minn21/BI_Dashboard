...
interface OccupancyRateFullScreenModalProps {
  isOpen: boolean;
  onClose: () => void;
  data: { 
    current: Array<{ name: string; value: number }>; 
    historical: Array<{ label: string; occupied: number; available: number }>;
  };
  title: string;
}

export function OccupancyRateFullScreenModal({ isOpen, onClose, data, title }: OccupancyRateFullScreenModalProps) {
  ...
  const COLORS = ['#10B981', '#F59E0B']; // Green for Occupied, Orange for Available
  ...
  const handleAnalyzeWithAI = async () => {
    ...
    try {
      const analysisResult = await geminiService.analyzeChartData(
        'occupancy',
        activeTab === 'current' ? data.current : data.historical.map(item => ({ name: item.label, value: item.occupied + item.available })),
        activeTab === 'current' ? 'Current Occupancy Analysis' : 'Historical Occupancy Trend Analysis'
      );
      ...
      setAnalysis({
        ...
        insight: 'Error occurred while analyzing occupancy data.',
          ...
          <ResponsiveContainer width="100%" height={chartHeight}>
            {historicalChartType === 'line' ? (
              <LineChart data={data.historical}>
                ...
                <Line type="monotone" dataKey="available" stroke="#F59E0B" name="Available" />
              </LineChart>
            ) : (
  ...
  const renderAnalysisContent = () => {
    if (isAnalyzing) {
      return <div className="text-gray-300">AI is analyzing occupancy data...</div>;
    }
    if (!analysis) return null;
    return (
      <div className="p-6 bg-gray-800 rounded-lg text-gray-300">
        <h3 className="text-xl text-blue-400 mb-4">AI Occupancy Analysis</h3>
          ...
          {!showAnalysis && (
            <div className="flex justify-center mt-6">
              ...
              >
                ...
                <span>Analyze Occupancy Data with AI</span>
              </button>
            </div>
          )}
          {showAnalysis && (
