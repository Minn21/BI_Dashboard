...
interface AgeGroupFullScreenModalProps {
  isOpen: boolean;
  onClose: () => void;
  data: { current: Array<{ name: string; value: number }>; historical: Array<{ label: string; child: number; adult: number; middle_age: number; elder: number }> };
  title: string;
}

export function AgeGroupFullScreenModal({ isOpen, onClose, data, title }: AgeGroupFullScreenModalProps) {
  ...
  const handleAnalyzeWithAI = async () => {
    ...
    try {
      const analysisResult = await geminiService.analyzeChartData(
        'ageGroups',
        activeTab === 'current' ? data.current : data.historical.map(item => ({ name: item.label, value: item.child + item.adult + item.middle_age + item.elder })),
        activeTab === 'current' ? 'Current Age Group Analysis' : 'Historical Age Group Trend Analysis'
      );
      ...
      setAnalysis({
        ...
        insight: 'Error occurred while analyzing age data.',
          ...
          <ResponsiveContainer width="100%" height={chartHeight}>
            {historicalChartType === 'line' ? (
              <LineChart data={data.historical}>
                ...
                <Tooltip />
                <Legend />
                <Line type="monotone" dataKey="child" stroke="#3B82F6" name="Child" />
                <Line type="monotone" dataKey="adult" stroke="#10B981" name="Adult" />
                <Line type="monotone" dataKey="middle_age" stroke="#F59E0B" name="Middle Age" />
                <Line type="monotone" dataKey="elder" stroke="#EF4444" name="Elder" />
              </LineChart>
            ) : (
              <BarChart data={data.historical}>
                ...
                <Tooltip />
                <Legend />
                <Bar dataKey="child" fill="#3B82F6" name="Child" />
                <Bar dataKey="adult" fill="#10B981" name="Adult" />
                <Bar dataKey="middle_age" fill="#F59E0B" name="Middle Age" />
                <Bar dataKey="elder" fill="#EF4444" name="Elder" />
              </BarChart>
            )}
          </ResponsiveContainer>
        </div>
  ...
  const renderAnalysisContent = () => {
    if (isAnalyzing) {
      return <div className="text-gray-300">AI is analyzing age data...</div>;
    }
    if (!analysis) return null;
    return (
      <div className="p-6 bg-gray-800 rounded-lg text-gray-300">
        <h3 className="text-xl text-blue-400 mb-4">AI Age Group Analysis</h3>
          ...
          {!showAnalysis && (
            <div className="flex justify-center mt-6">
              ...
              >
                ...
                <span>Analyze Age Data with AI</span>
              </button>
            </div>
          )}
          {showAnalysis && (
